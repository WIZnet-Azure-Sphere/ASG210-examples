...
// named constants such as WIZNET_ASG_EVB_USER_SW which are used when opening the peripherals, rather
...
typedef enum {
    ...
    ExitCode_Init_Led1 = 9,
    ExitCode_Init_Led3 = 10,
    ExitCode_Init_Led2 = 11,
...
// User configuration.
const char *const currentNetInterface = NET_INTERFACE_ETHERNET;
...
// - InterfaceUp: (WIZNET_ASG_EVB_STATUS_LD1)
// - ConnectedToNetwork: (WIZNET_ASG_EVB_STATUS_LD3) + (WIZNET_ASG_EVB_STATUS_LD1) ==> YELLOW
// - IpAvailable: (WIZNET_ASG_EVB_STATUS_LD2)
// - ConnectedToInternet: (WIZNET_ASG_EVB_STATUS_LD3)
static int ipUnassignedLed1GpioFd = -1;
static int ipAssignedLed2GpioFd = -1;
static int inetAvailableLed3GpioFd = -1;
...
/// </summary>
/// <param name="led1"> Turns ON (true) or OFF (false) the LED1.</param>
/// <param name="led2"> Turns ON (true) or OFF (false) the LED2.</param>
/// <param name="led3"> Turns ON (true) or OFF (false) the LED3.</param>
static void SetLedStates(bool led1, bool led2, bool led3)
{
    if (ipUnassignedLed1GpioFd != -1) {
        GPIO_SetValue(ipUnassignedLed1GpioFd, led1 ? GPIO_Value_Low : GPIO_Value_High);
    }
    if (ipAssignedLed2GpioFd != -1) {
        GPIO_SetValue(ipAssignedLed2GpioFd, led2 ? GPIO_Value_Low : GPIO_Value_High);
    }
    if (inetAvailableLed3GpioFd != -1) {
        GPIO_SetValue(inetAvailableLed3GpioFd, led3 ? GPIO_Value_Low : GPIO_Value_High);
    }
}
...
static void ClosePeripheralsAndHandlers(void)
{
    ...
    CloseFdAndPrintError(ipUnassignedLed1GpioFd, "ipUnassignedLed1GpioFd");
    CloseFdAndPrintError(ipAssignedLed2GpioFd, "ipAssignedLed2GpioFd");
    CloseFdAndPrintError(inetAvailableLed3GpioFd, "inetAvailavleLedGreenGpioFd");
}
...
static ExitCode InitPeripheralsAndHandlers(void)
{
    ...
    releaseIpButtonGpioFd = GPIO_OpenAsInput(WIZNET_ASG_EVB_USER_SW);
    ...
    ipUnassignedLed1GpioFd =
        GPIO_OpenAsOutput(WIZNET_ASG_EVB_STATUS_LD1, GPIO_OutputMode_PushPull, GPIO_Value_Low);
    if (ipUnassignedLed1GpioFd == -1) {
        Log_Debug("ERROR: Could not open LED1 as output: %s (%d).\n", strerror(errno), errno);
        return ExitCode_Init_Led1;
    }
    ipAssignedLed2GpioFd =
        GPIO_OpenAsOutput(WIZNET_ASG_EVB_STATUS_LD2, GPIO_OutputMode_PushPull, GPIO_Value_High);
    if (ipAssignedLed2GpioFd == -1) {
        Log_Debug("ERROR: Could not open LED2 as output: %s (%d).\n", strerror(errno), errno);
        return ExitCode_Init_Led2;
    }
    inetAvailableLed3GpioFd =
        GPIO_OpenAsOutput(WIZNET_ASG_EVB_STATUS_LD3, GPIO_OutputMode_PushPull, GPIO_Value_High);
    if (inetAvailableLed3GpioFd == -1) {
        Log_Debug("ERROR: Could not open LED2 as output: %s (%d).\n", strerror(errno), errno);
