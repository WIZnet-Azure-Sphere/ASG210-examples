...
static void AdcPollingEventHandler(EventLoopTimer *timer)
{
    ...
    uint32_t value;
    int result = ADC_Poll(adcControllerFd, WIZNET_ASG_EVB_ADC, &value);
    if (result == -1) {
...
static ExitCode InitPeripheralsAndHandlers(void)
{
    ...
    adcControllerFd = ADC_Open(WIZNET_ASG_EVB_ADC);
    if (adcControllerFd == -1) {
        Log_Debug("ADC_Open failed with error: %s (%d)\n", strerror(errno), errno);
        return ExitCode_Init_AdcOpen;
    }

    sampleBitCount = ADC_GetSampleBitCount(adcControllerFd, WIZNET_ASG_EVB_ADC_VAR);
    if (sampleBitCount == -1) {
    ...
    int result = ADC_SetReferenceVoltage(adcControllerFd, WIZNET_ASG_EVB_ADC_VAR,
                                         sampleMaxVoltage);
    if (result == -1) {
    ...
    struct timespec adcCheckPeriod = {.tv_sec = 0, .tv_nsec = 1000 * 1000};
    adcPollTimer =
