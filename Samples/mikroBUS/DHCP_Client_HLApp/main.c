...
static int mB1SPInResetGpioFd = -1;
static int mB1SPInSelGpioFd = -1;
static int mB2SPInResetGpioFd = -1;
static int mB2SPInSelGpioFd = -1;
...
static ExitCode InitPeripheralsAndHandlers(void)
{
    ...
    Log_Debug("Opening WIZNET_ASG_EVB_MB1_SPI_SEL as output.\n");
    mB1SPInSelGpioFd = GPIO_OpenAsOutput(WIZNET_ASG_EVB_MB1_SPI_SEL, GPIO_OutputMode_PushPull, GPIO_Value_Low);
    sleep(1);
    if (mB1SPInSelGpioFd == -1)
    {
        Log_Debug("ERROR: Could not open WIZNET_ASG_EVB_MB1_SPI_SEL GPIO: %s (%d).\n", strerror(errno), errno);
        return ExitCode_Init_Led;
    }

    Log_Debug("Opening WIZNET_ASG_EVB_MB1_RST as output.\n");
    mB1SPInResetGpioFd = GPIO_OpenAsOutput(WIZNET_ASG_EVB_MB1_RST, GPIO_OutputMode_PushPull, GPIO_Value_Low);
    sleep(1);
    if (mB1SPInResetGpioFd == -1)
    {
        Log_Debug("ERROR: Could not open WIZNET_ASG_EVB_MB1_RST GPIO: %s (%d).\n", strerror(errno), errno);
        return ExitCode_Init_Led;
    }

    Log_Debug("Opening WIZNET_ASG_EVB_MB2_SPI_SEL as output.\n");
    mB2SPInSelGpioFd = GPIO_OpenAsOutput(WIZNET_ASG_EVB_MB2_SPI_SEL, GPIO_OutputMode_PushPull, GPIO_Value_Low);
    sleep(1);
    if (mB2SPInSelGpioFd == -1)
    {
        Log_Debug("ERROR: Could not open WIZNET_ASG_EVB_MB2_SPI_SEL GPIO: %s (%d).\n", strerror(errno), errno);
        return ExitCode_Init_Led;
    }

    Log_Debug("Opening WIZNET_ASG_EVB_MB2_RST as output.\n");
    mB2SPInResetGpioFd = GPIO_OpenAsOutput(WIZNET_ASG_EVB_MB2_RST, GPIO_OutputMode_PushPull, GPIO_Value_Low);
    sleep(1);
    if (mB2SPInResetGpioFd == -1)
    {
        Log_Debug("ERROR: Could not open WIZNET_ASG_EVB_MB2_RST GPIO: %s (%d).\n", strerror(errno), errno);
        return ExitCode_Init_Led;
    }
...
void InitPrivateNetInfo(void)
{
    ...
    result = GPIO_SetValue(mB1SPInResetGpioFd, GPIO_Value_High);
    if (result != 0)
    {
        Log_Debug("ERROR: Could not set mB1SPInResetGpioFd output value: %s (%d).\n", strerror(errno), errno);
    }

    result = GPIO_SetValue(mB2SPInResetGpioFd, GPIO_Value_High);
    if (result != 0)
    {
        Log_Debug("ERROR: Could not set mB2SPInResetGpioFd output value: %s (%d).\n", strerror(errno), errno);
    }
    sleep(1);
